...
class TranscritorApp:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            header_frame,
            text="🎙️ Transcritor de Áudio/Vídeo",
        ...
        subtitle_label = ttk.Label(
            header_frame,
            text="Transcreva seus arquivos de áudio e vídeo com facilidade",
            font=("Segoe UI", 9),
            foreground="gray"
        )
        ...
        file_icon_label = ttk.Label(file_frame, text="📄", font=("Segoe UI", 24))
        file_icon_label.grid(row=0, column=0, rowspan=2, padx=(0, 15))
        ...
        self.btn_process.grid(row=0, column=1)
        
        # ===== SEÇÃO 2: Progresso com Estatísticas =====
        ...
        # Container para barra e porcentagem
        ...
        # Estatística 1: Trecho atual
        stat1_frame = ttk.Frame(stats_frame)
        stat1_frame.grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        ...
        # Estatística 2: Tempo estimado
        stat2_frame = ttk.Frame(stats_frame)
        stat2_frame.grid(row=0, column=1, sticky=tk.W, padx=(0, 10))
        ...
        # Estatística 3: Caracteres transcritos
        stat3_frame = ttk.Frame(stats_frame)
        stat3_frame.grid(row=0, column=2, sticky=tk.W)
        ...
        # ScrolledText com fonte monospace
        ...
        # Configurar tags de cores no log
        ...
        self.btn_clear.grid(row=1, column=0, sticky=tk.E)
        ...
        if ffmpeg_path:
            ffprobe_path = ffmpeg_path.replace("ffmpeg.exe", "ffprobe.exe").replace("ffmpeg", "ffprobe")
            if os.path.exists(ffprobe_path):
                footer_text = f"✅ FFmpeg e FFprobe detectados"
                footer_style = 'Success.TLabel'
            else:
                footer_text = f"⚠️ FFmpeg OK, mas falta ffprobe.exe"
        ...
        footer_label.grid(row=1, column=0, sticky=tk.W)
        ...
        if ffmpeg_path:
            ...
            if os.path.exists(ffprobe_path):
                ...
                self.log(f"⚠️ FFprobe não encontrado", "warning")
        else:
    ...
    def atualizar_tempo_decorrido(self):
        """Atualiza o tempo decorrido durante o processamento"""
        if self.processing and self.inicio_processamento:
            tempo_decorrido = datetime.datetime.now() - self.inicio_processamento
            horas, resto = divmod(tempo_decorrido.seconds, 3600)
            minutos, segundos = divmod(resto, 60)
            self.tempo_label.config(text=f"{horas:02d}:{minutos:02d}:{segundos:02d}")
    ...
    def selecionar_arquivo(self):
        ...
        if filename:
            ...
            tamanho = os.path.getsize(filename) / (1024 * 1024)  # MB
            self.file_info_label.config(
                text=f"{tipo} • {extensao.upper()} • {tamanho:.2f} MB",
                foreground="gray"
            )
            
            self.btn_process.config(state=tk.NORMAL)
            self.status_label.config(text="✅ Arquivo carregado - Pronto para processar", style='Success.TLabel')
    ...
    def carregar_audio(self, caminho_arquivo: str) -> AudioSegment:
        """Carrega áudio de qualquer formato suportado"""
        ...
        try:
            if extensao == 'wav':
                ...
                self.log(f"🎬 Extraindo áudio do vídeo...", "info")
                audio = AudioSegment.from_file(caminho_arquivo)
            else:
                audio = AudioSegment.from_file(caminho_arquivo)
            
            self.log(f"✅ Áudio carregado com sucesso!", "success")
            ...
            for i in range(num_trechos):
                ...
                with open(transcription_file, "a", encoding="utf-8") as f:
                    ...
                    if texto_concatenado:
                        ...
                        f.write("[Sem transcrição válida]\n")
                
                self.log(f"✅ Trecho {i+1} concluído ({len(texto_concatenado)} caracteres)", "success")
            
            self.log(f"🎉 Processamento concluído!", "success")
    ...
    def finalizar_processamento(self, sucesso: bool, info: str):
        """Finaliza o processamento e mostra resultado"""
